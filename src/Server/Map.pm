...
func view($dbh, $id, $res) {
    ...
    my $game_ids = $dbh->selectall_arrayref("select id, round, finished, array (select faction || ' ' || vp from game_role where faction != 'admin' and game=game.id order by vp desc) as factions from game where base_map=? order by finished, round, id",
                                            { Slice => {} },
                                            $id);
