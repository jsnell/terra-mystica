...
my $idle_players = $dbh->selectall_arrayref("select game, array_agg(faction) as factions from game_role where game in (select id from game where not finished and not aborted and last_update < now() - interval '1 weeks' and round > 0) and (action_required or leech_required) and faction not like 'player%' and game like ? group by game",
                                            { Slice => {} },
                                            $id_pattern);
