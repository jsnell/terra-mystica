...
sub get_finished_game_results {
    ...
    my $rows = $dbh->selectall_arrayref(
        "select game, faction, vp, rank, start_order, email.player, email, game.player_count from game_role left join game on game=game.id left join email on email=email.address where faction != 'admin' and game.finished and game.round=6 and (exclude_from_stats is null or exclude_from_stats = false)",
        {});
