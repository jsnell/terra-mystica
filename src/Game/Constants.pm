...
Readonly our %actions => (
	...
	ACT7 => { cost => { PW => 4 }, 
			  subaction => { move => 1 } },
	ACT8 => { cost => { PW => 4 }, gain => { SP = > 1 } },
	ACT9 => { cost => { PW => 5, P => 1 }, gain => { GAIN_FAVOR => 1 } },
...
Readonly our %tiles => init_tiles (
    ...
    BON6 => { income => { W => 2 },
			  ...
			  exclude_option => 'merchants-features' },
    BON7 => { income => { W => 1 },
	...
	BON11 => { income => { W => 2 },
			   pass_vp => { SA => [0, 4], SH => [0, 4], SY => [0, 4] },
			   option => 'merchants-features' },
	BON12 => { income => { W => 1, C => 1 }, 
			   pass_vp => { SP => [ map { $_ * 3 } 0..3 ] }, 
			   option => 'merchants-features' },
	BON13 => { income => { SP => 1 },
			   option => 'merchants-features' },
	BON14 => { income => {}, vp => { TRADE => 1 }, 
			   option => 'merchants-features' }, 
	...
	FAV13 => { gain => { FIRE => 1 }, income => {}, 
			   subaction => { fav13choice => 1 },
			   option => 'merchants-features' },
	FAV14 => { gain => { WATER => 1 }, income => { SP => 1 },
			   option => 'merchants-features' },
	FAV15 => { gain => { EARTH => 1 }, income => {},
			   special => { better_trades => 1 },
			   option => 'merchants-features' },
	FAV16 => { gain => { AIR => 1 }, income => {}, 
			   pass_vp => { TM => [ 0, 2, 2, 2, 3, 3, 3, 4, 4, 4, 5, 5, 5, 5, 5, 5 ] },
			   option => 'merchants-features' },
	...
	SCORE10 => { vp => { map(("TW$_", 5), 1..10) },
                ...
                option => 'merchants-features',
                income => { C => 1 } },
	SCORE11 => { vp => { ship => 2, exchange => 2 },
                vp_display => 'SHIP/DIG >> 2',
                vp_mode => 'gain',
                cult => 'WATER',
                req => 2,
                option => 'merchants-features',
                income => { C => 2 } },
	SCORE12 => { vp => { SA => 5, SH => 5, SY => 5 },
                vp_display => 'SA/SH/SY >> 5',
                vp_mode => 'build',
                cult => 'EARTH',
                req => 2,
                option => 'merchants-features',
    ...
    TW7 => { gain => { KEY => 1, VP => 4, GAIN_SHIP => 1, carpet_range => 1 },
             option => 'mini-expansion-1', exclude_option => 'merchants-features' },
    TW8 => { gain => { KEY => 1, VP => 11 }, count => 1, option => 'mini-expansion-1' },
	TW9 => { gain => { KEY => 1, VP => 4, GAIN_SHIP => 1, carpet_range => 1 },
			 option => 'merchants-features' },
	TW10 => { gain => { KEY => 1, VP => 3, SP => 2 }, 
			  option => 'merchants-features' }
);
