...
sub get_game_metadata {
    ...
    my ($rows) =
        $dbh->selectall_arrayref("select extract(epoch from now() - last_update) as time_since_update, game.description, finished, aborted, game_options, player_count, wanted_player_count, base_map as map_variant, game_options.deadline_hours, admin_user, array[array_agg(game_active_time.player), array_agg(game_active_time.active_seconds)::text[]] as active_times from game left join game_options on game.id=game_options.game left join game_active_time on game_active_time.game=game.id where game.id=? group by last_update, game.description, finished, aborted, game_options, player_count, wanted_player_count, base_map, game_options.deadline_hours, admin_user",
                                 { Slice => {} },
                                 $id);
...
sub get_open_game_list {
    ...
    my $games = $dbh->selectall_arrayref(
        "select game.id, game.player_count, game.wanted_player_count, game.description, array(select player from game_player where game_player.game=game.id) as players, game.game_options, game_options.minimum_rating, game_options.maximum_rating, game_options.deadline_hours, game.base_map as map_variant from game left join game_options on game.id=game_options.game where game.wanted_player_count is not null and game.player_count != game.wanted_player_count and not game.finished",
        { Slice => {} }
        );
...
sub get_game_list_by_pattern {
    ...
    my $res = $dbh->selectall_arrayref(
        "select id, finished, aborted, round, turn, array_agg(game_role.faction) as factions, array_agg(game_role.faction_player) as usernames, array_agg(game_role.rank) as ranks, array_agg(game_role.vp) as vps, (extract(epoch from now() - game.last_update)) as seconds_since_update from game join game_role on game.id=game_role.game where id like ? group by id, finished, aborted, last_update, round limit 1000",
        { Slice => {} },
        $ids);
