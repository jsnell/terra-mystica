...
function renderFactionStats(count, elem) {
    ...
    $("faction-stats" + count).insert("<tr><td>#{key}<td>#{value.wins}<td>#{value.count}<td>#{value.win_rate}<td>#{value.average_position}<td>#{value.average_vp}<td>#{value.average_loss_vp}<td><span id=#{key}-links style='display: none'>#{value.game_links}</span><a href='javascript:showLinks(\"#{key}\")' id=#{key}-show-link>[show]</a></tr>"
                                      .interpolate(elem));
}
...
function renderHighScoresForFaction(faction) {
    ['standard', 'non-standard'].each(function(standard) {
        ...
        row.insert(new Element("td").updateText(faction));
        for (var i = 3; i <= 5; ++i) {
