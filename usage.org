...
*** =!skip=, =!dontskip=, =!continue=

Comment out blocks of commands. Can be nested.
...
If you have multiple separate alternative plans, you can put them all
in a =!skip= block and activate one of them by changing that to a
=!dontskip=.
...
=!skip= and =!dontskip= blocks can be nested. In this case every =!continue=
will be close the most recently opened block that has not yet been closed.
Commands will be ignored if there is at least one open =!skip= block.
...
!skip
  ...
  !dontskip
    this isn't executed either
  !continue
  none of this is executed
!continue
#+END_EXAMPLE

The following is an error, the second =!continue= does not have a matching
opening =!skip= or =!dontskip=.
#+BEGIN_EXAMPLE
!skip
!continue
!continue
