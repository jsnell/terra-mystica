...
For now you'll need to ask Juho to create a game, either through BGG
GeekMail (username jsnell) or email (jsnell@iki.fi). Please tell
whether this is going to be a PBF game on BGG, or if communication
will happen through some other method. This information is needed to
name the game correctly.

You'll get two links in response: one for viewing the game, one for
editing it. Anyone with the second link can change the game in
arbitrary ways, so you might not want to share it too widely.

After the faction selection is done, the admin page of the game will
